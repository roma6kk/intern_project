    ...
    return (
      <div className="min-h-screen bg-muted/50 flex items-center justify-center p-4">
        <div className="bg-card rounded-lg shadow-sm border border-border p-8 flex flex-col items-center gap-4 max-w-sm w-full">
          ...
          <p className="text-muted-foreground font-medium text-center">Войдите для поиска постов</p>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {!loading && posts.length === 0 && (
          <div className="bg-card rounded-lg shadow-sm border border-border p-8 text-center">
            ...
            <h3 className="text-lg font-semibold text-muted-foreground mb-2">{query ? 'Посты не найдены' : 'Постов нет'}</h3>
            <p className="text-sm text-muted-foreground">{query ? 'Попробуйте изменить поисковый запрос' : 'Пока нет доступных постов'}</p>
          </div>
        )}
        ...
        {!hasMore && normalizedPosts.length > 0 && (
          <div className="text-center py-6">
            <p className="text-sm text-muted-foreground">Вы просмотрели все результаты</p>
          </div>
        )}
      </div>
