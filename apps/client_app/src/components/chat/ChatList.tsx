...
export default function ChatList({ chats, selectedChatId, onSelectChat }: ChatListProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-white border-r">
              ...
              >
                <div className="w-12 h-12 rounded-full bg-gray-200 overflow-hidden flex-shrink-0 flex items-center justify-center">
                  {avatarUrl ? (
                    ...
                    chat.type === 'GROUP' ? <Users size={20} className="text-gray-500"/> : <User size={20} className="text-gray-500"/>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-500 truncate">{chatName}</h3>
