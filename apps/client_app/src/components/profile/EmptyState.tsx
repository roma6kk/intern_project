...
export default function EmptyState({ type }: EmptyStateProps) {
  ...
  if (type === 'private') {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-gray-500 text-lg font-semibold mb-2">{config?.text}</p>
        {config?.subtext && <p className="text-gray-400 text-sm text-center">{config.subtext}</p>}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="text-gray-500 text-lg">{config?.text}</p>
    </div>
  );
}
